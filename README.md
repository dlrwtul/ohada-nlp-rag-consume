...
- **Notebook de soumission** : `notebook/NLP_project_ollama.ipynb` construit désormais, en plus de la réponse, un fichier `submission.csv` au format attendu par la Phase 1 (colonnes `ID`/`Target`, 3 lignes par question : `{ID}_Answer`, `{ID}_Document_de_Référence`, `{ID}_Numéro_d'Article`). Le document de référence et le numéro d'article sont extraits par une regex (`extraire_reference`) sur le premier document récupéré, qui repère les motifs `ARTICLE <numéro> <ABRÉVIATION>` présents dans le corpus (ex. `AUPSRVE`, `AUA`) — une heuristique simple, pas une garantie d'exactitude ; sur certaines décisions le mot capté juste après "ARTICLE <n>" est un mot de liaison ("DE", "DU", "TRAITE") plutôt qu'une vraie abréviation d'Acte Uniforme.
- **Jeu de test synthétique + métrique Phase 2** : n'ayant accès ni à `Test.csv`/`Eval.csv` du concours ni à `zindi.world` depuis cet environnement, le notebook génère son propre jeu de test à partir du corpus OHADA lui-même (`Test_synthetique.csv`) : pour chaque décision où une citation d'article est repérable, une question est dérivée de l'intitulé de l'analyse ("Quelle est la position de la jurisprudence OHADA sur : ... ?"), avec le document/article de référence connu directement (extrait de cette même décision) et un extrait du jugement comme réponse de référence approximative. La métrique proposée pour la Phase 2 (`calculer_metrique`) combine ROUGE-1 sur la réponse (50%, poids repris de la Phase 1) et l'exactitude du document de référence et du numéro d'article prédits (25% + 25%) ; le notebook boucle sur ce jeu synthétique et écrit `Evaluation_synthetique.csv`. **Ce jeu de test est un substitut auto-généré, pas les vraies questions du concours** : les scores qu'il produit servent à vérifier que le code tourne, pas à juger la qualité réelle du modèle — à remplacer par `Test.csv`/`Eval.csv` dès qu'accessibles.
